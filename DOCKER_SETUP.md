...
This will:
- ✅ Build the Java backend with Java 25
- ✅ Start MySQL database
- ✅ Initialize the database schema with `init.sql`
- ✅ Start the game server on port `7777`
...
├── database/
│   ├── docker-compose.yml      ← Old database-only compose (deprecated)
│   └── init.sql               ← Database schema
├── java-backend/
│   ├── Dockerfile             ← Java backend container definition
│   ├── pom.xml               ← Maven dependencies
│   ├── src/                  ← Java source code
│   └── target/               ← Compiled JAR (generated by Docker)
└──poc-vampire-raiders-multiplayer/
    └── (Godot project - run in editor or export)
```
...
### Making Java Changes
1. Edit code in `java-backend/src/`
